...
// Function to create a super admin user (one-time use)
const createSuperAdmin = async () => {
  try {
    ...
    // Create super admin
    const phone = "9999999999"; // Change this to desired phone number
    const password = "admin@123"; // Change this to a strong password
    const name = "Super Admin";
    const email = "admin@example.com";
    const token = md5(phone + Date.now());

    await connection.execute(
      "INSERT INTO users (phone, password, name, email, token, level, status, veri, ip_address, created_at) VALUES (?, ?, ?, ?, ?, 1, 1, 1, ?, NOW())",
      [phone, md5(password), name, email, token, "127.0.0.1"],
    );
...
// Uncomment and run this once to create a super admin
// createSuperAdmin();
...
const totalJoin = async (req, res) => {
  ...
  try {
    ...
    if (rows.length > 0) {
      ...
      try {
        const [wingoallResult] = await connection.query(
      ...
      try {
        const [winGo1Result] = await connection.execute(
      ...
      try {
        const [winGo10Result] = await connection.execute(
