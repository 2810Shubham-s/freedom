...
//     try {
//         const [users] = await connection.query('SELECT * FROM users WHERE token = ? AND level = 0', [auth]);
...
//     try {
//         const { username, password, telegram, cskh } = req.body;
...
//         const [existing] = await connection.query('SELECT * FROM admin WHERE username = ?', [username]);
...
//             'INSERT INTO admin (username, password, telegram, cskh) VALUES (?, ?, ?, ?)',
//             [username, md5(password), telegram, cskh || '']
//         );
...
//     try {
//         const [admins] = await connection.query('SELECT id, username, telegram, cskh FROM admin');
